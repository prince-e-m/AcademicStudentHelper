...
static void editCourse(Course &c, std::function<void()> clearConsole) {
    while (true) {
        ...
        std::cout << "[4] Prediction / Goal Calculator\n";
        std::cout << "[0] Back\n";
        std::cout << "Choice: ";
        ...
        if (sub == 1) {
            ...
            // Prediction and goal calculator
            float projected = GradeMath::ComputeProjectedFinal(c);
            std::cout << "Projected grade (using current entries): " << std::fixed << std::setprecision(2) << projected << "\n";
            ...
            if (goal <= 0.0f) {
                ...
                float req = GradeMath::RequiredFinalForGoal(c, goal);
                if (req == FLT_MAX) {
                    std::cout << "Cannot compute required final: final weight is zero.\n";
                }
                else if (req > 100.0f) {
                    std::cout << "Required final to reach " << goal << "% is " << req << "% (IMPOSSIBLE >100%).\n";
                }
                else if (req <= 0.0f) {
                    std::cout << "Goal already achieved with current grades. No additional points needed.\n";
                }
                else {
                    std::cout << "You need a final exam grade of at least " << std::fixed << std::setprecision(2) << req << "% to reach " << goal << "% overall.\n";
                }
                waitForEnter();
            }
        }
    ...
    while (true) {
        ...
        if (mode == 9) {
            if (confirmPrompt("This will delete ALL saved profiles. Continue?")) {
                if (Persistence::DeleteAllStudentFiles(std::filesystem::path("data"))) std::cout << "All profiles deleted.\n";
